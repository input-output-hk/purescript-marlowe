...
instance encodeJsonTimeout :: EncodeJson Timeout where
  ...
  encodeJson (TimeValue time) = instantToJson time

instance decodeJsonTimeout :: DecodeJson Timeout where
  decodeJson json =
    lmap (Named "Timeout")
      $ caseJsonObject
          ( lmap (Named "constructor TimeValue")
              $ TimeValue <$> instantFromJson json
          )
...
instance showTimeout :: Show Timeout where
  show (TimeValue time) = show $ round $ unwrap $ unInstant time
  show v = genericShow v

instance toCoreTimeout :: ToCore Timeout Instant where
...
instance contractHasChoices :: HasChoices Contract where
  ...
  getChoiceNames (Pay _ _ _ val cont) =
    getChoiceNames val <> getChoiceNames cont
  getChoiceNames (If obs cont1 cont2) = getChoiceNames obs
    <> getChoiceNames cont1
    <> getChoiceNames cont2
  getChoiceNames (When cases _ cont) =
